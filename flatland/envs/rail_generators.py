...
def rail_from_file_with_cities(filename, load_from_package=None) -> RailGenerator:
    """
    Utility to load pickle file with cities

    Parameters
    ----------
    filename : Pickle file generated by env.save() or editor and manual additions

    Returns
    -------
    function
        Generator function that always returns a GridTransitionMap object with
        the matrix of correct 16-bit bitmaps for each rail_spec_of_cell and a dict agents_hints
        :param filename:
        :param load_from_package:
        :return:
    """
    ...
    def generator(width: int, height: int, num_agents: int, num_resets: int = 0,
        ...
        rail = GridTransitionMap(width=np.shape(grid)[1], height=np.shape(grid)[0], transitions=rail_env_transitions)
        rail.grid = grid
        if b"distance_map" in map_data.keys():
            distance_map = map_data[b"distance_map"]
            if len(distance_map) > 0:
                return rail, {'distance_map': distance_map}

        return [rail, hints_dict]
